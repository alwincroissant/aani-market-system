...
class AdminDashboardController extends Controller
{
    public function index()
    {
        ...
        // Get top products across entire market
    ...
    public function getOrdersData(Request $request)
    {
        $query = DB::table('orders as o')
            ...
            ->join('orderline as ol', 'o.id', '=', 'ol.orderinfo_id')
            ->join('item as i', 'ol.item_id', '=', 'i.id')
            ->select(
                'o.orderinfo_id',
                'c.customer_name',
                DB::raw('SUM(ol.quantity * i.sell_price) as total'),
                DB::raw('DATE(o.date_placed) as order_date')
            )
            ->groupBy('o.orderinfo_id', 'c.customer_name', 'o.date_placed');
